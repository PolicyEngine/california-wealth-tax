    ...
    case "timing":
      body = (
        <div className="space-y-5">
          ...
          />
          <p className="text-xs leading-5 text-[var(--gray-500)]">
            Applied after nominal receipts are converted to 2026 dollars, and to
            the real income-tax loss stream. Both start in 2027. Rauh et al.
            use real rates of 1.5% to 4.5%.
          </p>
          <Slider
            ...
            format={percent(1)}
            quickPicks={[
              { label: "Constant prices (0%)", value: 0 },
              { label: "Illustrative 2%", value: 0.02 },
            ]}
          />
          <p className="text-xs leading-5 text-[var(--gray-500)]">
            The default assumes constant prices (0% inflation); it is not a
            forecast. Inflation deflates nominal wealth-tax receipts before
            real discounting. The income-tax stream is already in real terms.
          </p>
          <Field
            title="Payment"
            note="Installments carry a 7.5% nondeductible charge on the unpaid balance (RTC §50301(c)): $115 per $100 of liability in nominal receipts. They exceed the lump sum in present value when the effective nominal discount rate, (1 + real discount rate) × (1 + inflation) − 1, is below 7.5%."
          >
            <div className="flex flex-wrap gap-2">
              <ToggleChip
