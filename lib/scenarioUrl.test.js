...
describe("scenarioUrl helpers", () => {
  ...
  it("reads a stage-two key with no pit flag as an old link with effects on", () => {
    for (const query of ["income=2", "yield=0.015", "horizon=25", "annual_return=0.05"]) {
      const parsed = parseScenarioParams(new URLSearchParams(query), DEFAULT_PARAMS);
      expect(parsed.includeIncomeTaxEffects, query).toBe(true);
    }
    // Current links say pit=0, which wins.
    const current = parseScenarioParams(new URLSearchParams("yield=0.015&pit=0"), DEFAULT_PARAMS);
    expect(current.includeIncomeTaxEffects).toBe(false);
    expect(current.incomeYieldRate).toBe(0.015);
  });

  it("gives an old link with effects on the uniform-yield method however it says so", () => {
    const defaults = { ...DEFAULT_PARAMS, incomeTaxMethod: "filings", cohortIncomeTaxB: 4.3 };
    const rauhEra = parseScenarioParams(
      new URLSearchParams(
        `date=2025-10-17&base=${WEALTH_BASES.AFTER_PRE_SNAPSHOT_DEPARTURES}&exre=1&avoidance=0&unannounced=0.48&yield=0.02`
      ),
      defaults
    );
    expect(rauhEra.includeIncomeTaxEffects).toBe(true);
    expect(rauhEra.incomeTaxMethod).toBe("yield");
    expect(rauhEra.incomeYieldRate).toBe(0.02);

    const current = parseScenarioParams(new URLSearchParams("pit=1&pitmethod=wealth"), defaults);
    expect(current.incomeTaxMethod).toBe("wealth");
  });
