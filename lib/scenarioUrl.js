...
export function parseScenarioParams(searchParams, defaultParams) {
  ...
  // with a uniform yield. New links always carry pitmethod when pit=1, so a
  // link with income-tax effects and no method is an old one.
  if (
    "incomeTaxMethod" in defaultParams &&
    searchParams.get("pitmethod") == null &&
    (searchParams.get("pit") === "1" || searchParams.get("pit") === "true")
  ) {
  ...
  // Links written before `pit` existed carried stage-two settings only when
  // income-tax effects were on; they are recognizable by their legacy keys.
  // Current links always say `pit` when a stage-two setting differs from its
  // default, so nothing else implies it.
  const isLegacyLink = ["base", "baseline", "return", "income"].some(
