...
const ENUM_PARAM_CONFIG = {
  ...
  residencyExclusionIds: {
    queryKey: "exclude",
    type: "csv",
    normalize: normalizeResidencyExclusionIds,
  },
  incomeTaxMoverIds: {
    queryKey: "movers",
    type: "csv",
    normalize: normalizeIncomeTaxMoverIds,
  },
  ...
  incomeTaxMode: {
    queryKey: "pitmode",
    values: Object.values(INCOME_TAX_MODES),
  },
};
...
const INCOME_TAX_PARAM_KEYS = new Set([
  "incomeTaxMoverIds",
  "incomeTaxMode",
  "annualReturnRate",
  "incomeYieldRate",
...
function isInactiveIncomeTaxParam(key, params) {
  if (!params.includeIncomeTaxEffects && INCOME_TAX_PARAM_KEYS.has(key)) {
    return true;
  }

  const usesFtbAggregate =
    params.incomeTaxMode === INCOME_TAX_MODES.FTB_AGGREGATE;

  if (key === "incomeYieldRate" && usesFtbAggregate) {
    return true;
  }

  return key === "aggregateAnnualIncomeTaxB" && !usesFtbAggregate;
}
...
export function parseScenarioParams(searchParams, defaultParams) {
  ...
  for (const [key, config] of Object.entries(ENUM_PARAM_CONFIG)) {
    ...
    if (config.type === "csv") {
      params[key] =
        rawValue === "none"
          ? []
          : config.normalize(rawValue.split(",").filter(Boolean));
      continue;
    }
  ...
  if (
    !hasExplicitPitFlag &&
    (searchParams.get("movers") != null ||
      searchParams.get("pitmode") != null ||
      searchParams.get("pitc") != null ||
...
export function serializeScenarioParams(params, defaultParams) {
  ...
  for (const [key, config] of Object.entries(ENUM_PARAM_CONFIG)) {
    ...
    if (config.type === "csv") {
      if (isInactiveIncomeTaxParam(key, params)) {
        continue;
      }

      const serializedValue = config.normalize(value ?? []).join(",");
      const serializedDefault = config.normalize(defaultValue ?? []).join(",");

      if (serializedValue !== serializedDefault) {
        searchParams.set(config.queryKey, serializedValue || "none");
      }
      continue;
    }
