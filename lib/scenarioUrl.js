...
const PARAM_CONFIG = {
  ...
  cohortIncomeTaxB: {
    queryKey: "cohortpit",
    // The slider's floor; below the filings-based amounts ($0.25B) the
    // filings method could not divide the total.
    min: 1,
    max: 10,
    step: 0.05,
    decimals: 2,
  ...
  migrationSemiElasticity: {
    ...
    min: 0,
    max: 30,
    // Two decimals so Rauh et al.'s 10.32 survives a shared link.
    step: 0.01,
    decimals: 2,
  },
  annualReturnRate: {
...
export function parseScenarioParams(searchParams, defaultParams) {
  ...
  for (const [key, config] of Object.entries(PARAM_CONFIG)) {
    ...
    // The slider's top stop is the perpetuity; keep the URL, the slider, the
    // group card and the model on one value.
    if (key === "horizonYears" && params[key] >= config.max) {
      params[key] = Infinity;
    }
  }

  // Links written before `pit` existed carried a stage-two setting only when
  // income-tax effects were on, some of them with no other legacy key. Current
  // links write `pit=0` whenever a stage-two setting appears with effects off,
  // so a stage-two key with no `pit` at all can only be one of those.
  if (
    !hasExplicitPitFlag &&
    ["annual_return", "yield", "horizon", "return", "income"].some(
  ...
  // Links shared before the cohort methods existed priced movers' income tax
  // with a uniform yield. New links always carry pitmethod when effects are
  // on, so effects on (however the link says it) with no method is an old one.
  if (
    "incomeTaxMethod" in defaultParams &&
    searchParams.get("pitmethod") == null &&
    params.includeIncomeTaxEffects
  ) {
    params.incomeTaxMethod = "yield";
  }
...
export function serializeScenarioParams(params, defaultParams) {
  ...
  for (const [key, config] of Object.entries(PARAM_CONFIG)) {
    ...
    // Older scenario objects predate some parameters; leave those out.
  ...
  if (params.includeIncomeTaxEffects && params.incomeTaxMethod) {
    searchParams.set("pitmethod", params.incomeTaxMethod);
  }

  // A stage-two setting in the link with income-tax effects off says so
  // explicitly, so the link reads the same way whoever opens it.
  const stageTwoKeys = ["yield", "horizon", "annual_return", "attrib", "igrowth", "cohortpit"];
  if (
    !params.includeIncomeTaxEffects &&
    !searchParams.has("pit") &&
    stageTwoKeys.some((key) => searchParams.has(key))
  ) {
    searchParams.set("pit", "0");
  }
