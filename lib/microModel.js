...
// CA top marginal rate is 13.3%. At billionaire income levels the
// effective rate is 13.28–13.31% — constant for all practical purposes.
const CA_EFFECTIVE_INCOME_TAX_RATE = 0.133;
...
export function computeMicroResults({
  ...
  const rows = billionaires.map((b) => {
    ...
    const annualIncomeTaxB = annualIncomeB * CA_EFFECTIVE_INCOME_TAX_RATE;
