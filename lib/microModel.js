...
export const VALUATION_SOURCES = {
  ...
  // On the roster, and the all-states list for this date is available, but
  // Forbes no longer lists them. Forbes lists net worth of $1 billion or more,
  // so its current estimate is below the threshold: no tax under the rate ramp.
 ...
 *      Forbes has since moved elsewhere; a roster member absent from that list
 *      is below $1 billion per Forbes and leaves the base;
 ...
 * list who were not yet billionaires on January 1 are added and flagged
 * `residencyAssumed`: the threshold applies on the valuation date, and their
 * January 1 residency is inferred from their current Forbes listing.
 */
    ...
    .map((residencyRow) => {
      ...
      if (anyStateValuation) {
        return {
          ...
          ...anyStateValuation,
          ...residencyFlags,
          valuationSource: VALUATION_SOURCES.ANY_STATE_LIST,
          valuationFallback: false,
        };
      }
...
export function computeMicroResults({
  ...
  const rows = billionaires.map((b) => {
    ...
    const inBase = inSelectedBase && !offForbesList;
