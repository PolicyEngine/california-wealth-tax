...
export const INCOME_TAX_MODES = {
  YIELD: "yield",
  FTB_AGGREGATE: "ftbAggregate",
};
// Rauh et al. (2026) §5.1 estimate annual California personal income tax from
// the billionaire cohort using FTB tax statistics and a Pareto tail fit:
// $5.8B/yr if billionaires are the top income filers, $3.3B/yr after a Monte
// Carlo relaxation of the wealth-income rank correlation.
export const FTB_AGGREGATE_INCOME_TAX_RANGE_B = {
  min: 3.3,
  max: 5.8,
  midpoint: 4.55,
};
...
export function computeMicroResults({
  ...
  const incomeTaxMoverNameSet = new Set(incomeTaxMoverNames ?? []);
  const usingExplicitExclusions = excludedNames.length > 0 || !wealthBase;
  const usingExplicitIncomeTaxMovers = Array.isArray(incomeTaxMoverNames);
  ...
  let rows = billionaires.map((b) => {
    ...
    const countsTowardKnownIncomeTaxLoss = usingExplicitIncomeTaxMovers
      ? incomeTaxMoverNameSet.has(b.name)
  ...
  const isInCorrectedBase = (row) =>
    usingExplicitExclusions
      ? row.includeInRawForbes &&
        (!row.excludedFromWealthTaxBase ||
          row.departureTiming === "pre_snapshot")
      : !row.excludeFromCorrectedBase;
  // Income-tax allocation universe: the corrected billionaire roster plus any
  // explicitly counted movers, mirroring Rauh et al.'s "all 212 billionaires"
  // income-tax base (their NPV term is f·C, with f the movers' wealth share).
  const isInIncomeTaxUniverse = (row) =>
    isInCorrectedBase(row) || row.countsTowardKnownIncomeTaxLoss;
  const incomeTaxUniverseWealthB = rows.reduce(
    (sum, row) => (isInIncomeTaxUniverse(row) ? sum + row.netWorthB : sum),
    0
  );

  if (incomeTaxMode === INCOME_TAX_MODES.FTB_AGGREGATE) {
    rows = rows.map((row) => ({
      ...row,
      annualIncomeTaxB:
        isInIncomeTaxUniverse(row) && incomeTaxUniverseWealthB > 0
          ? aggregateAnnualIncomeTaxB *
            (row.netWorthB / incomeTaxUniverseWealthB)
          : 0,
    }));
  }

  const rawForbesRows = rows.filter((row) => row.includeInRawForbes);
  const correctedBaseRows = rows.filter(isInCorrectedBase);
  ...
  const incomeTaxUniverseAnnualIncomeTaxB = rows.reduce(
    (sum, row) =>
      isInIncomeTaxUniverse(row) ? sum + row.annualIncomeTaxB : sum,
    0
  );
  ...
  return {
    ...
    incomeTaxUniverseWealthB,
    incomeTaxUniverseAnnualIncomeTaxB,
  };
}
