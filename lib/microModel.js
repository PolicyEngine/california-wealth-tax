 ...
 * stored January 1 list is a backfill from per-person Forbes histories, so it
 * lacks anyone Forbes added to its real-time list later (most of them with the
 * annual list in March 2026) and anyone whose state was blank that day. Both
...
export function computeMicroResults({
  ...
  let rows = billionaires.map((b) => {
    ...
    // Someone kept in the base is a resident and keeps paying income tax; the
    // income-tax loss follows the user's residency choices, not the flags.
    // Someone removed as never a resident was never paying California income
    // tax, so nothing is lost when they go.
  ...
  if (incomeTaxMethod !== INCOME_TAX_METHODS.YIELD) {
    ...
    const inCohort = (row) => row.includeInRawForbes && !row.belowThreshold;
    const filedTaxB = (row) =>
    ...
    rows = rows.map((row) => {
      if (!inCohort(row)) {
        return { ...row, annualIncomeB: 0, annualIncomeTaxB: 0, incomeTaxSource: null };
      }
