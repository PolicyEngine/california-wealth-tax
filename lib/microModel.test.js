...
describe("computeMicroResults", () => {
  ...
  describe("modeled further departures", () => {
    ...
    it("never takes the modeled share from people reported to have left after January 1", () => {
      const results = computeMicroResults({
        billionaires: liveRows,
        incomeTaxLookup,
        excludeRealEstate: true,
      ...
      const zuckerberg = results.rows.find((row) => row.name === "Mark Zuckerberg");

      expect(zuckerberg.inBase).toBe(true);
      expect(zuckerberg.eligibleForUnannouncedDeparture).toBe(false);
    });
  });
});
