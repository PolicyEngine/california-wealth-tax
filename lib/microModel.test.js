...
describe("computeMicroResults", () => {
  ...
  describe("modeled further departures", () => {
    ...
    it("loses no income tax for someone removed as never a resident", () => {
      const results = computeMicroResults({
        billionaires: liveRows,
        incomeTaxLookup,
        excludedNames: ["Drew Houston", "Lynsi Snyder", "Larry Page"],
        neverResidentNames: ["Drew Houston", "Lynsi Snyder"],
        excludeRealEstate: true,
        incomeYieldRate: 0.02,
      });
      const byName = Object.fromEntries(results.rows.map((row) => [row.name, row]));

      expect(byName["Drew Houston"].inBase).toBe(false);
      expect(byName["Drew Houston"].countsTowardKnownIncomeTaxLoss).toBe(false);
      expect(byName["Lynsi Snyder"].countsTowardKnownIncomeTaxLoss).toBe(false);
      expect(byName["Larry Page"].countsTowardKnownIncomeTaxLoss).toBe(true);
    });
