...
export function computePaperNumbers({
  ...
  const bridge = berkeleyToHooverBridge((params) =>
  ...
  return {
    ...
    baseExclusionCount: rosterRows.filter(
...
export function manuscriptStrings(numbers) {
  ...
  return {
    ...
    baseExclusionCount: `${word(numbers.baseExclusionCount)} roster member is excluded from every base`,
    residencyAdjustmentCount: `${word(numbers.residencyAdjustmentCount)} documented claims`,
