...
export const INCOME_TAX_MOVER_ADJUSTMENTS = [
  ...RESIDENCY_ADJUSTMENTS.filter(
    (adjustment) => adjustment.category === "pre_snapshot_departure"
  ),
  {
    id: "sacks",
    name: "David Sacks",
    category: "pre_snapshot_departure",
    summary:
      "Added from paper corrections rather than the raw Forbes California list; count only for future California income-tax effects if attributed to the tax episode.",
  },
  {
    id: "zuckerberg",
    ...
    summary:
      "Reported after the January 1 residency cutoff; count only for future California income-tax effects if attributed to the tax episode.",
  },
  {
    id: "koum",
    name: "Jan Koum",
    category: "reported_departure",
    summary:
      "Reported departure case; count only for future California income-tax effects if attributed to the tax episode.",
  },
  {
    id: "hastings",
    name: "Reed Hastings",
...
const INCOME_TAX_MOVER_BY_ID = new Map(
  INCOME_TAX_MOVER_ADJUSTMENTS.map((adjustment) => [adjustment.id, adjustment])
);

export const DEFAULT_INCOME_TAX_MOVER_IDS = INCOME_TAX_MOVER_ADJUSTMENTS.map(
...
export function normalizeIncomeTaxMoverIds(ids = []) {
  const seen = new Set();

  return INCOME_TAX_MOVER_ADJUSTMENTS.map((adjustment) => adjustment.id).filter(
    (id) => {
      if (!ids.includes(id) || seen.has(id)) {
        return false;
      }
...
export function incomeTaxMoverNamesFromIds(ids = []) {
  return normalizeIncomeTaxMoverIds(ids)
    .map((id) => INCOME_TAX_MOVER_BY_ID.get(id)?.name)
    .filter(Boolean);
}
