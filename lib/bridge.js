...
export function shapleyBridge({ start, end, factors, evaluate }) {
  ...
  return {
    ...
    contributions,
    // A perpetuity that does not converge makes every coalition value
    // infinite and every marginal effect NaN; callers say so instead.
    finite,
  };
}
...
export const ASSUMPTION_GROUPS = [
  ...
  {
    id: "migration",
    // People who left before January 1, 2026 without public notice. Leaving
    // after that date does not escape the tax (residency is fixed that day).
    label: "Unannounced departures before January 1",
    shortLabel: "Unannounced departures",
    keys: [
  ...
  {
    id: "incomeAllocation",
    label: "Who pays the cohort's income tax",
    shortLabel: "Who pays it",
    keys: ["incomeTaxMethod", "cohortIncomeTaxB", "incomeYieldRate"],
  },
  {
    id: "erosion",
  ...
  },
  {
    ...
    shortLabel: "Timing",
    keys: ["discountRate", "inflationRate", "wealthTaxPaymentMode"],
  },
];
...
export function scenarioBridge({ start, end, score }) {
  ...
  return {
    ...bridge,
    // A group whose keys differ but whose value is exactly zero in every
    // ordering (a stage-two setting with income-tax effects off) is not a
    // step. Anything nonzero stays, so the bars always sum to the gap.
    contributions: bridge.contributions.filter(
      (step) =>
        step.changed &&
        [step.value, step.min, step.max].some((value) => Math.abs(value) > 1e-9)
    ),
  };
}
