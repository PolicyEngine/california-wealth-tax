...
export function scenarioBridge({ start, end, score }) {
  ...
  return {
    ...bridge,
    // A group whose keys differ but whose value is zero in every ordering
    // (a stage-two setting with income-tax effects off) is not a step.
    contributions: bridge.contributions.filter(
      (step) =>
        step.changed &&
        (Math.abs(step.value) >= 0.05 ||
          Math.abs(step.min) >= 0.05 ||
          Math.abs(step.max) >= 0.05)
    ),
  };
}
