...
function presentValueOfReceipts(schedule, discountRate, inflationRate) {
  return schedule.reduce(
    (sum, entry) => sum + entry.receipt /
      Math.pow((1 + inflationRate) * (1 + discountRate), entry.year),
    0
  );
}
 ...
 * @param {number} params.inflationRate - Annual price inflation; zero assumes constant prices
