...
describe("buildAnnualCashFlows", () => {
  it("spreads wealth tax over installment years and starts PIT loss in year 1", () => {
    ...
    // Year 0 is 2026, the valuation year: nothing is payable yet.
    expect(result.rows[0].netCashFlow).toBeCloseTo(0, 6);
    ...
    // Year 1 (2027): first 20% principal installment and first PIT loss
    ...
    // Year 2: second installment carries the 7.5% charge on the unpaid balance
    expect(result.rows[2].wealthTaxReceipt).toBeCloseTo(20.8, 6);

    // Year 6: no more wealth tax receipt
    expect(result.rows[6].wealthTaxReceipt).toBeCloseTo(0, 6);
