...
export function scoreScenario({ params, rows, incomeTaxLookup, sourceDate }) {
  ...
  const neverResidentNames = neverResidentNamesFromIds(
    params.residencyExclusionIds
  );
  const microInputs = {
    billionaires: rows,
    ...
    incomeTaxMethod: params.incomeTaxMethod,
    cohortIncomeTaxB: params.cohortIncomeTaxB,
